...
Для обхода подобного ограничения в синтаксическом анализаторе решено использовать обертку вокруг каналов Go - структуру [`TransactionChannel[T any]`](internal/channel/transaction.go). Она предоставляет следующие методы:
